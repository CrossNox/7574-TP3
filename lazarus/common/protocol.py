...
from lazarus.constants import NO_SESSION
...
class MessageType(enum.Enum):
    ...
    NOTAVAIL = (
        10  # Enviado por el servidor, para indicar que no se encuentra disponible
    )
...
LOG_TABLE = {
    ...
    MessageType.SYN: "SYN",
    MessageType.SYNCHECK: "SYNCHECK",
    MessageType.RESULT: "RESULT",
    MessageType.FIN: "FIN",
    # Server Msgs
    MessageType.SYNACK: "SYNACK",
    MessageType.CHECKACK: "CHECKACK",
    MessageType.RESRESP: "RESRESP",
    MessageType.NOTDONE: "NOTDONE",
    MessageType.FINACK: "FINACK",
    MessageType.INVALMSG: "INVALMSG",
    MessageType.NOTAVAIL: "NOTAVAIL",
    MessageType.INVALSESSION: "INVALSESSION",
    MessageType.REDIRECT: "REDIRECT",
    MessageType.PROBEACK: "PROBEACK",
}
...
class ClientMsg(ProtocolMessage):
    def __init__(self, mtype: MessageType, session_id: int = NO_SESSION):
        super().__init__(mtype)
        self.session_id = session_id

    def encode(self) -> str:
        m = {"type": self.mtype.value, "session_id": self.session_id}
        return json.dumps(m)
...
class ServerMsg(ProtocolMessage):
    def __init__(self, mtype: MessageType, payload: Optional[Dict] = None):
        super().__init__(mtype)
        self.payload = payload or {}

    def encode(self) -> str:
        m = {"type": self.mtype.value, "payload": self.payload}
        return json.dumps(m)
